...
static int ttyACM0 = -1; // pseudo terminal used for communication with the IDE
static fd_set fdSet;
...
int recvBytes(uint8 *buf, int count) {
	...
	FD_ZERO(&fdSet);
	FD_SET(ttyACM0, &fdSet);
	tv.tv_sec = 0;
	tv.tv_usec = 0;
	int ret = select(ttyACM0+1, &fdSet, NULL, NULL, &tv);
	if (ret == -1) {
		perror("select()");
	} else if (ret) {
		readCount = read(ttyACM0, buf, count);
		if (readCount < 0) {
			readCount = 0;
...
int sendByte(char aByte) {
	int written = write(ttyACM0, &aByte, 1);
	if (written < 0) {
		written = 0;
...
int sendBytes(uint8 *buf, int start, int end) {
	...
	tv.tv_sec = 0;
	tv.tv_usec = 0;
	int ret = select(ttyACM0+1, NULL, &fdSet, NULL, &tv);
	if (ret == -1) {
		perror("select()");
	} else if (ret) {
		writtenBytes = write(ttyACM0, &buf[start], end - start);
		if (writtenBytes < 0) {
			writtenBytes = 0;
...
void setupTtyACM0Connection(void) {
	ttyACM0 = open("/dev/ttyACM0", O_NOCTTY | O_NONBLOCK | O_RDWR | O_SYNC);
	if (ttyACM0 < 0) {
		perror("setup: open()");
	}
	...
	tcgetattr(ttyACM0, &settings);
	cfmakeraw(&settings);
	cfsetispeed(&settings, B115200);
	cfsetospeed(&settings, B115200);
	settings.c_cc[VMIN] = 0;
	settings.c_cc[VTIME] = 0;
	tcsetattr(ttyACM0, TCSANOW, &settings);
	tcflush(ttyACM0, TCIOFLUSH);
}
